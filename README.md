...
By: Cody Bunnell Web 3.0 Developer ( ApexExpress )

! Statement Declaration @ Id Command
# Ctrl Command $ Keyvalue Objective Q or L

% Split Keypair DATA ^ Peek Seek in DATA
& Runtime while DATA * Registar DATA

( Open Parameter ) Close Parameter
_ Nested Level + Add or Book

( ) = Eq or Value - Tac or Minus
!!/ Repeat Command @@/ Id Command

##/ Ctrl Command $$/'' Value str Command
%%/ Split Command ^^/ Peek Command

&&/Runtime goto next if pass test logical boolean

*(Registar) was no Zero )
__Nested ++Incremental

## Verification for Token Langauge

Token Block Matrix BEGIN: DATA SET
!-!)) !-!)) for while !! @@ ## $$ %% ^^ && ** (( ~~ ))

Verification TRANSIT DATA MATRIX BLOCK

'!@#$%^&*(!) !-!)'
'!) @) #) $) %) ^) &) *) () !) - ()'
'(!(@(#($(%(^(&(*((!)))))))))) () - !))'

for while my DATA @set=error.ext*->$value:=>#@@@
